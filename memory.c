...
/* malloc'ed chunck of memory */
/* last used position in above */
...
noreturn initMemoryManager()
{
	...
	for (i = 0; i < ObjectTableMax; i++) {
		...
		size = objectTable[i].size;
		if (size < 0)
			size = ((-size) + 3) / 4;
		if (size != 0) {
			objectTable[i].memory = memoryBlock + (int)(objectTable[i].memory);
		} else
			objectTable[i].memory = (object *) 0;
	}
	printf("Visit symbols\n");
	visit(symbols);
	printf("set freelist\n");
	setFreeLists();
	printf("End init memory\n");
#else
